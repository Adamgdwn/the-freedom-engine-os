...
const server = createServer(async (req, res) => {
  try {
    ...
    if (method === "POST" && url.pathname === "/api/mobile-companion") {
      ...
      if (isReplyRequest(parsed)) {
        sendJson(res, 200, await requestMobileCompanionReply(parsed));
        return;
      }

      if (isSummaryRequest(parsed)) {
        sendJson(res, 200, await requestMobileCompanionSummary(parsed));
        return;
      }
    ...
    if (method === "GET" && url.pathname === "/api/mobile-companion/speech") {
      const text = readGatewaySpeechTextHeader(req.headers);
